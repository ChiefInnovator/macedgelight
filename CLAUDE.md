...
- **DisplayBrightnessManager** — XDR brightness boost via invisible Metal EDR overlay (alpha=0, headroom signaling) + linear gamma table scaling (1.45x). Constants: `maxHeadroomCap`, `gammaScale`. Sleep/wake handled in `EdgeLightManager`: `willSleep` tears down the boost (without touching `settings.edrBoosted`), `didWake` calls `resetGammaToProfile()` to wipe any dirty LUT and re-activates the boost after a 2s settle if the setting was on.
